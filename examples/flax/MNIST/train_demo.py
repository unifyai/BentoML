...
class CNN(nn.Module):
    ...
    def setup(self) -> None:
        ...
        print('num class is ', self._num_class)
        return super().setup()
    @nn.compact
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:  # pylint: disable=W0221
        ...
        x = nn.relu(x)
        x = nn.Dense(features=self._num_class)(x)
        return x
...
if __name__ == "__main__":
    ...
    tag1 = bentoml.flax.save_model("mnist_flax_1", CNN(), training_state)
    tag2 = bentoml.flax.save_model("mnist_flax_2", CNN(num_class=10), training_state)
    print(f"Saved model1: {tag1}")
    print(f"Saved model2: {tag2}")
